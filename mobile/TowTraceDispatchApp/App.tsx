...
export default function App() {
  ...
  if (!token) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>TowTrace Dispatch App</Text>
  ...
  return (
    <AuthContext.Provider value={authValue}>
      <NavigationContainer>
        <Stack.Navigator
          initialRouteName={userRole === "dispatcher" ? "FleetTracker" : "JobTracker"}
          screenOptions={{
            headerStyle: {
              backgroundColor: '#4287f5', // Different color from Driver app
            },
            headerTintColor: '#fff',
            headerTitleStyle: {
              fontWeight: 'bold',
        ...
        >
          {userRole === "dispatcher" ? (
            ...
            // Driver screens (should be redirected to driver app)
            <>
              <Stack.Screen 
                name="JobTracker" 
                component={JobTrackerPlaceholder} 
                options={{ title: "Job Tracker (Unavailable)" }}
              />
              <Stack.Screen 
                name="VINScanner" 
                component={VINScannerPlaceholder} 
                options={{ title: "VIN Scanner (Unavailable)" }}
              />
              <Stack.Screen 
                name="FleetTracker" 
                component={FleetTracker} 
...
// Placeholder components for inaccessible screens in Dispatcher App
const JobTrackerPlaceholder = () => (
  <View style={styles.container}>
    <Text style={styles.title}>Not available in Dispatch App</Text>
    <Text>The Job Tracker feature is only available in the Driver App.</Text>
  </View>
);

const VINScannerPlaceholder = () => (
  <View style={styles.container}>
    <Text style={styles.title}>Not available in Dispatch App</Text>
    <Text>The VIN Scanner feature is only available in the Driver App.</Text>
  </View>
);