...
const VINScanner: React.FC = () => {
  ...
  const loadOfflineQueue = async () => {
    try {
      ...
      if (queueData) {
        setOfflineQueue(JSON.parse(queueData) as QueueItem[]);
      }
    } catch (error) {
  ...
  const saveOfflineQueue = async (newQueue: QueueItem[]) => {
    try {
    ...
    try {
      ...
      vehiclePhotos.forEach((photo, index) => {
        ...
        const filename = photo.uri.split('/').pop() || `photo_${index}.jpg`;
              ...
              // Use type assertion for FormData compatibility with React Native
          ...
          <>
            ...
            <View style={styles.photoGrid}>
              {photos.map((photo, index) => (
                <View key={`photo-${photo.timestamp}`} style={styles.photoContainer}>
