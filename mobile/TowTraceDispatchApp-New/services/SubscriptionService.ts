...
export class SubscriptionService {
  ...
  /**
   * Get feature usage for the current tenant (Enterprise feature)
   * @returns Promise resolving to feature usage data
   */
  static async getFeatureUsage(): Promise<any> {
    try {
      const token = await AsyncStorage.getItem('authToken');
      if (!token) throw new Error('Not authenticated');
      
      const response = await axios.get(
        `${API_BASE_URL}/api/overwatch/feature/usage`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  /**
   * Get users for the current tenant with their subscription feature access
   * @returns Promise resolving to user list with feature access
   */
  static async getUsersWithFeatureAccess(): Promise<any[]> {
    try {
      const token = await AsyncStorage.getItem('authToken');
      if (!token) return [];
      
      const response = await axios.get(
        `${API_BASE_URL}/api/overwatch/users/features`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
      ...
      console.error('Error fetching users with feature access:', error);
      return [];
    }
  }
}